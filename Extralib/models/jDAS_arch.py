...
class UNet:
    ...
    def __init__(self):
        self.kernel = (3, 5)
        self.f0 = 2 # n x n filter
        self.N_blocks = 4  # what is N_blocks?
        self.use_bn = True
        self.use_dropout = True
        self.dropout_rate = 0.1
        self.AA = False #anti-alasing
        self.LR = 5e-4
    ...
    def conv_layer(self, x, filters, kernel_size,
        ...
        """
        use_bias = True #adding an additional degree of freedom?
        if use_bn:
            use_bias = False
    ...
    def construct(self):
        ...
        f = self.f0
        kernel = self.kernel
        use_bn = self.use_bn #batch normalization
        use_dropout = self.use_dropout #drop-out
        AA = self.AA #anti-aliasing
        activation = self.activation
        data_shape = self.data_shape

        input = Input(data_shape)  #2 times the input
        print('This is jDAS UNET Input')
        print(input.shape)
        print(input)
        mask_input = Input(data_shape)  #Check the input 
        print('This is jDAS UNET Mask input')
        ...
        x = c_mask_input * x   #Predicted output
        model = Model([input, mask_input], x) #2 input only
        #model = Model(sample, x)  #Just the input
