...
class JDAS:
    ...
    def _J_construct(model, data, Nsub):
        ...
        result = model.predict((eval_samples, masks)) #input: eval_samples * masks, output: 1 traces --> 0.00001
        rec = np.sum(result, axis=1)[:, :, 0] #Take the middle of the traces only?
        _ = gc.collect()

        return rec


    def denoise(self, data, postfilter=False, filter_band=(1, 250, 2000), verbose=True):
